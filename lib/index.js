...
__mergeDirectory = function(combine) {
  var folderFiles, includedFiles, outputFileText;
  ...
  if (combine.isExclude) {
    console.log("IS EXCLUDE", folderFiles.length);
    ...
    if (combine.include || combine.includeRegex) {
      ...
      console.log("IS INCLUDE", includedFiles.length);
      if (combine.include) {
  ...
  console.log("COMBINING FILES", folderFiles);
